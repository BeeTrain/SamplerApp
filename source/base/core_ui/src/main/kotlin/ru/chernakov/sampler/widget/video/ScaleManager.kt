...
class ScaleManager(private val mViewSize: Size, private val mVideoSize: Size) {
    ...
    private fun getMatrix(sx: Float, sy: Float, pivotPoint: PivotPoint): Matrix {
        return when (pivotPoint) {
            ...
            PivotPoint.CENTER_BOTTOM -> getMatrix(sx, sy, mViewSize.width / 2f, mViewSize.height.toFloat())
            PivotPoint.RIGHT_TOP -> getMatrix(sx, sy, mViewSize.width.toFloat(), 0f)
            PivotPoint.RIGHT_CENTER -> getMatrix(sx, sy, mViewSize.width.toFloat(), mViewSize.height / 2f)
            PivotPoint.RIGHT_BOTTOM -> getMatrix(sx, sy, mViewSize.width.toFloat(), mViewSize.height.toFloat())
        }
    }
